...
$res_cons = SysGetList('tbl_events', [], "AND id='".$get_id."'");
if(count($res_cons) <= 0){
	echo "Không có dữ liệu";
	exit();
}
$row = $res_cons[0];
$res_cates = SysGetList('tbl_event_group', [], "AND id='".$row['cat_id']."'");
$res_cate = $res_cates[0];
$cate_link = ROOTHOST.$res_cate['alias'];

$str_ids_parent = str_replace('_', ',', $res_cate['path']);
$res_children = SysGetList('tbl_event_group', [], " AND `id` IN(".$str_ids_parent.")");
				...
				<div class="col-md-8 col-lg-9">
					<div class="header-content">
						<ol class="breadcrumb" style="margin-bottom: 5px;">
							<li class="breadcrumb-item">Sự kiện</li>
							<?php
							foreach ($res_children as $key => $value) {
								$bre_title = $value['title'];
								$bre_link = ROOTHOST.'su-kien-'.$value['alias'];
					...
					<?php
					$res_relateds = SysGetList('tbl_events', [], "AND id <> ".$get_id." AND cat_id=".$res_cate['id']." AND isactive=1 LIMIT 0,3");
					if(count($res_relateds)>0){
						?>
						<div class="widget-related-posts event-related">
							<?php
							foreach ($res_relateds as $key => $value) {
								$rel_link = ROOTHOST.'su-kien-'.$value['alias'].'-'.$value['id'];
